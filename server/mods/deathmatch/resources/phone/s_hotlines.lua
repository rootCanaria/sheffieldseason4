local easyHotlines = {
	[911] = {
		...
		dialogue = {
			...
			local query = exports.mysql:query_insert_free("INSERT INTO `mdc_calls` (`caller`,`number`,`description`) VALUES ('" ..callstate.name.. "','" .. caller.phone .. "','" .. exports.mysql:escape_string(zonelocation .. " - " .. tostring(callstate.emergency)) .. "')")
			if query then
	...
	[311] = {
		name = "LSPD Non-Emergency",
		order = 10,
		factions = { 1, 142, 50 }, 
		require_radio = true,
		operator = "LSPD Operator", 
		dialogue = {
	...
	[2504] = {
		...
		require_phone = true,
		dialogue = {
			{ q = "Hello, Cargo Group. here, how can we help you?", as = "reason" },
			{ q = "What is your name?", as = "name" },
			done = "Thanks for the call, an employee should call back soon!"
		},
		done = function(caller, callstate, players)
			players:send({
				"SMS from Cargo Group.: " .. callstate.name .. " is requesting assistance: " .. callstate.reason .. ".",
				"Call the customer back on phone #" .. caller.phone .. "."
			}, 120, 255, 80)
		end
	},
