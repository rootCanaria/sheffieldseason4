	...
	fetchRemote(apiURL, options, function(data, errno)
		if not errno.success then
			...
			if table and table.success and table.data then
				...
				if size > 200000 then
					triggerClientEvent(player, 'clothes:wizard2Result', resourceRoot, 'File exceeds the maximum size of 200KB')
				elseif height > 1024 or width > 1200 then
					triggerClientEvent(player, 'clothes:wizard2Result', resourceRoot, 'File exceeds the maximum dimensions of 1024x1200')
				else
					-- Initial Checks done
					local options = {
						queueName = "clothes",
