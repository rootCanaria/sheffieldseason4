...
function drawWindowGUI(element, windowTexture)
	if not element or not isElement(element) then
		...
		if guiGetVisible(element) then
			...
			if getElementData(element, "ui:text") ~= guiGetText(element) then
				for i, v in ipairs(renderlist) do
					if v[1] == element then
						...
						roundedRectangleh(0, 0, width, height, tocolor(20, 20, 20, 180),_) -- main window
						roundedRectanglesh(0, 0, width, 18, tocolor(20, 20, 20, 200),_) -- titlebar
						dxDrawLine(0, 19, width, 19, tocolor(255, 192, 0, 255), 2.0)
			...
			roundedRectangleh(x, y, width, height, tocolor(20, 20, 20, 180),_)
	...
	if element and isElement(element) then
		if guiGetVisible(element) then
			...
			for i, v in ipairs(getElementChildren(element)) do
				...
				roundedRectanglesh(x + difference, y, width, height+1, tocolor(20, 20, 20, 200),_)
				...
				if guiGetSelectedTab(element) == v then
					dxDrawLine(x+difference, y+20, x+difference+tabwidth, y+20, tocolor(255, 192, 0, 255), 1.0)
				end

				difference = difference+dxGetTextWidth(text, 1.0, "default")+19
			end

			roundedRectangleh(x, y + 22, panelWidth, panelHeight - 22, tocolor(20, 20, 20, 200), _)
		end
	end
end
...
function guiConvertButton(element, window)
	...
	if buttonTexture and buttonHoverTexture and buttonClickTexture then
		...
		roundedRectangleh(0, 0, width, height, tocolor(255, 192, 0, 255),_)
		...
		roundedRectangleh(0, 0, width, height, tocolor(255, 192, 0, 200),_)
		dxDrawText(text, 0, 0, width, height, tocolor(255, 255, 255, 240), 1.0, "default", "center", "center", true, true)

		dxSetRenderTarget(buttonClickTexture)
		roundedRectangleh(0, 0, width, height, tocolor(255, 192, 0, 100),_)
		dxDrawText(text, 0, 0, width, height, tocolor(255, 255, 255, 240), 1.0, "default", "center", "center", true, true)

		dxSetRenderTarget(buttonDisabledTexture)
		roundedRectangleh(0, 0, width, height, tocolor(80, 80, 80, 225),_)
	...
	if element and isElement(element) then
		if guiGetVisible(element) then
			...
			if guiGetProperty(element, "Disabled") == "True" then
				roundedRectangleh(x, y, width, height, tocolor(20, 20, 20, 225), _)
				dxDrawImage(x, y, width, height, textures[4], 0, 0, 0, color)
			else
				if isCursorWithinRectangle(x, y, width, height) then
					if getKeyState("mouse1") then
						roundedRectangleh(x, y, width, height, tocolor(255, 192, 0, 100), _)
						dxDrawImage(x, y, width, height, textures[3], 0, 0, 0, color)
					else
						roundedRectangleh(x, y, width, height, tocolor(255, 192, 0, 200), _)
						dxDrawImage(x, y, width, height, textures[2], 0, 0, 0, color)
					end
				else
					roundedRectangleh(x, y, width, height, tocolor(255, 192, 0, 225), _)
...
function roundedRectangleh(x, y, w, h, borderColor, bgColor, postGUI)
...
function roundedRectanglesh(x, y, w, h, borderColor, bgColor, postGUI)
	if (x and y and w and h) then
		if (not borderColor) then
			borderColor = tocolor(0, 0, 0, 200);
		end
		...
		--> Background
		dxDrawRectangle(x, y, w, h-2, bgColor, postGUI);
