...
local PD_VEHICLES = { 44, 490, 528, 523, 598, 596, 597, 599, 601 }
...
function main ( )
	...
	if canAccess( source, 'canSeeCalls' ) then
		local callsResult = exports.mysql:query( "SELECT * FROM mdc_calls" )
		 outputDebugString("Check 2")
		if ( callsResult ) then
			while true do
			    outputDebugString("Cant get row")
				row = exports.mysql:fetch_assoc( callsResult )
				if not row then break end
				outputDebugString("Inserted")
			...
			outputDebugString("Free Result Done")
		end
	end
