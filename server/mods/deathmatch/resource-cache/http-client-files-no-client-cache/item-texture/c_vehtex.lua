...
local extraVehTexNames = {
	...
	[599] = {"vehiclepoldecals128","vehiclepoldecals128lod"},

	[523] = {"vehiclepoldecals128","vehiclepoldecals128lod"},

	[427] = {"vehiclepoldecals128","vehiclepoldecals128lod"},
...
function vehTex_showGui(editVehicle)
	...
	exports.PInterface:convertToDx(gui.window)
...
function prvTexture(editVehicle)
	vehicle = editVehicle
	vehTex_addGui(true)
end
addEvent("item-texture:previewVehTex", true)
addEventHandler("item-texture:previewVehTex", root, prvTexture)
function vehTex_addGui(prv)
	...
	if not prv then
		titletext = "Add New Vehicle Texture"
	else
		titletext = "Preview Vehicle Texture"
	end
	gui.window2 = guiCreateWindow(634, 416, 456, 166, titletext, false)
	...
	if not prv then
		...
		addEventHandler ( "onClientGUIClick", gui.addApply, vehTex_addGui_applyPrv, false )
	end
	...
	local texnames = engineGetModelTextureNames(tostring(model))
			...
			table.insert(texnames, "vehiclegrunge256")
...
function vehTex_addGui_applyPrv()
	...
	if (not texname or texname == "" or texname == " ") then
		...
		outuputChatBox("You did not select what texture you want to replace.")
		return false
	...
	if not isURLValid(texurl) then
		...
		outuputChatBox("Invalid URL! Allowed filetypes are jpg and png.")
	...
	addTexture(vehicle, texname, texurl)
		
		setTimer(function()
			removeTexture(vehicle, texname)
		end, 60000,1)
		
		guiSetText(gui.addApply, "Sucssfuly preview texture")
